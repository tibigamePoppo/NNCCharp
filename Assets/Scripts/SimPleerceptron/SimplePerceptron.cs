...
{
    // 重みとバイアス
    private float[] _weights = { 0.0f, 0.0f }; // 2つの入力に対する重み
    ...
    void Start()
    {
        ...
        float[] outputs = { 0f, 0f, 0f, 1f }; // ANDゲートの出力
    ...
    void Train(float[,] inputs, float[] outputs, int epochs)
    {
        for (int epoch = 0; epoch < epochs; epoch++)
        {
            ...
            {
                float[] input = { inputs[i, 0], inputs[i, 1] };
    ...
    void Test(float[,] inputs)
    {
        ...
        Debug.Log($"Train result weight[0] = {_weights[0]}, weight[1] = {_weights[1]}, bias = {_bias}");

        for (int i = 0; i < inputs.GetLength(0); i++)
        {
            float[] input = { inputs[i, 0], inputs[i, 1] };
            float output = CalculateOutput(input);
            Debug.Log($"Input: {input[0]}, {input[1]} -> Output: {output}");
        }
    }
}
